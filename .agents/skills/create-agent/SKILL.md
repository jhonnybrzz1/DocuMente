...
Chain multiple agents where each agent processes the output of the previous one:
...
const analystAgent = createAgent({
  ...
  instructions: 'You are a business analyst. Analyze requirements and produce structured output.',
});
...
const techAgent = createAgent({
  apiKey: process.env.OPENROUTER_API_KEY!,
  model: 'openai/gpt-4o',
  instructions: 'You are a technical lead. Convert business requirements to technical specifications.',
});
...
const qaAgent = createAgent({
  apiKey: process.env.OPENROUTER_API_KEY!,
  model: 'anthropic/claude-3.5-sonnet',
  instructions: 'You are a QA specialist. Review specifications for completeness and testability.',
});

async function orchestrateAnalysis(userInput: string) {
  // Step 1: Business analysis
  const businessAnalysis = await analystAgent.sendSync(userInput);
  console.log('Business Analysis:', businessAnalysis);

  // Step 2: Technical specification
  const techSpec = await techAgent.sendSync(
    `Convert this business analysis to technical specs:\n${businessAnalysis}`
  );
  console.log('Technical Spec:', techSpec);

  // Step 3: QA review
  const qaReview = await qaAgent.sendSync(
    `Review this technical specification for completeness:\n${techSpec}`
  );
  console.log('QA Review:', qaReview);
...
```typescript
async function parallelAnalysis(userInput: string) {
  const agents = [
    createAgent({
      apiKey: process.env.OPENROUTER_API_KEY!,
      model: 'anthropic/claude-3.5-sonnet',
  ...
  const results = await Promise.all(
    agents.map(agent => agent.sendSync(userInput))
  );
...
### Agent with Classification Router

Route to different agents based on input classification:
...
function createRouterAgent(): AgentClassifier {
  const classifierAgent = createAgent({
    apiKey: process.env.OPENROUTER_API_KEY!,
    model: 'openrouter/auto',
    instructions: 'Classify the input as: "technical", "business", "creative", or "general". Return only the classification.',
  });
  ...
  const agents = {
    technical: createAgent({
      apiKey: process.env.OPENROUTER_API_KEY!,
      model: 'anthropic/claude-3.5-sonnet',
      instructions: 'You are a technical expert. Provide detailed technical analysis.',
    }),
    business: createAgent({
      apiKey: process.env.OPENROUTER_API_KEY!,
      model: 'openai/gpt-4o',
      instructions: 'You are a business consultant. Provide business insights.',
    }),
    creative: createAgent({
      apiKey: process.env.OPENROUTER_API_KEY!,
      model: 'google/gemini-pro-1.5',
      instructions: 'You are a creative writer. Provide creative content.',
    }),
    general: createAgent({
      apiKey: process.env.OPENROUTER_API_KEY!,
      model: 'openrouter/auto',
  ...
  return {
    async classify(input: string): Promise<string> {
      const classification = await classifierAgent.sendSync(input);
      const cleanClassification = classification.toLowerCase().trim();
      return agents[cleanClassification] ? cleanClassification : 'general';
    },
    agents,
  };
}

async function routedAgent(input: string) {
  const router = createRouterAgent();
  const classification = await router.classify(input);
  const agent = router.agents[classification];
  return await agent.sendSync(input);
}
```

### Cross-Validation Pattern

Use multiple agents to validate and improve outputs:

```typescript
async function crossValidate(input: string, validators: Agent[]) {
  // Primary agent generates initial response
  const primaryAgent = createAgent({
    apiKey: process.env.OPENROUTER_API_KEY!,
    model: 'anthropic/claude-3.5-sonnet',
    instructions: 'You are the primary analyst. Provide comprehensive analysis.',
  });

  const initialResponse = await primaryAgent.sendSync(input);

  // Validators review and suggest improvements
  const validationResults = await Promise.all(
    validators.map(agent =>
      agent.sendSync(`Review this analysis and suggest improvements:\n${initialResponse}`)
    )
  );

  // Synthesize feedback
  const synthesizerAgent = createAgent({
    apiKey: process.env.OPENROUTER_API_KEY!,
    model: 'openai/gpt-4o',
    instructions: 'Synthesize validation feedback into actionable improvements.',
  });

  const feedback = validationResults.join('\n\n---\n\n');
  const improvements = await synthesizerAgent.sendSync(
...
Implement session management for long-running conversations:
...
class AgentSessionManager {
  ...
  createSession(id: string, config: AgentConfig, metadata?: Record<string, unknown>): AgentSession {
...
class ResilientAgent {
  ...
  constructor(config: AgentConfig & { fallbackModels?: string[] }) {
    this.agent = createAgent(config);
    this.fallbackModels = config.fallbackModels || ['openrouter/auto'];
    this.retryConfig = { maxRetries: 3, backoffMs: 1000 };
  }

  async sendWithRetry(content: string): Promise<string> {
    let lastError: Error | null = null;

    for (let attempt = 0; attempt < this.retryConfig.maxRetries; attempt++) {
      try {
        return await this.agent.send(content);
      } catch (error) {
    ...
    for (const fallbackModel of this.fallbackModels) {
      try {
        ...
        this.agent.setInstructions(this.agent['config'].instructions);
        // Update model (this would require modifying the Agent class to support model switching)
        return await this.agent.send(content);
      } catch (error) {
...
class MonitoredAgent {
  ...
  async send(content: string): Promise<string> {
    ...
    try {
      const response = await this.agent.send(content);
...
## 🔗 Conexão com o AiChatFlow1

Esses padrões de orquestração e gerenciamento de ciclo de vida refletem exatamente a arquitetura operacional utilizada pelo sistema **AiChatFlow1** em produção:

- **Orquestração:** O sistema utiliza o parâmetro `agentExecutionOrder` para definir de forma determinística a sequência e dependência de execução dos agentes na esteira de refinamento.
- **Classificação:** O componente `demandClassifier` avalia o input bruto da demanda e escolhe quais agentes especializados serão invocados para atuar na esteira correspondente (ex: Técnico ou Negócios).
- **Validação Cruzada (Cross-Validation):** Agentes de validação (`validationAgents`) revisam, testam e garantem conformidade aos contratos de tipos (Type Adherence) de forma reativa nos outputs gerados pelas etapas anteriores.
- **Resiliência e Fallback:** O sistema possui as propriedades `fallbackUsed` e `fallbackReason` para registrar e orquestrar transições seguras entre LLMs em tempo de execução quando o modelo principal encontra restrições ou falhas.
